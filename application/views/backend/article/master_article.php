...
<head>
    ...
    <title>Master Article - Gereja Peduli Covid</title>
 			...
 			<div id="page-content">
                <div class="panel panel-primary"> 
                    <div class="panel-heading" style="background: #455a64">
                        <h3 class="panel-title"><b>Article</b></h3>
                    </div>
                    ...
                    <?php 
                          ...
                          ?>
                        <button style="background: #455a64;font-family:montserrat" class="btn btn-primary btn-labeled fa fa-plus" onclick="location.href='<?=site_url('admin/article/add')?>'">ADD NEW</button>                         
                    </div>  
                    <div>
                    ...
                    <table id="demo-dt-basic" class="table table-striped table-responsive table-bordered" cellspacing="0" width="100%">
                            ...
                            <tr style="background: #455a64; color:white">                         
                                ...
                                <th style="text-align:center">Type Article</th>                                                                                              
                                <th style="text-align:center">Media Type</th>
                                <th style="text-align:center">Created At</th>
                                <th style="text-align:center">Created By</th>
                                <th style="text-align:center">Reviewed On</th>
                             ...
                             if($list_article != FALSE){
                                    ...
                                    foreach($list_article->result() as $article){
                                      
                                      $status_review = "Pending";
                                      $color = "#ebaa4b";
									if($article->status_review == 1){
										$status_review = "Approved";
                                      $color = "green";
									}else if($article->status_review == 2){
                                ...
                                <td><?php echo $article->title; ?></td>
		                            <td><?php echo $article->type_article_name; ?></td>		                            
		                            <td><?php echo $article->media_type_name; ?></td>
		                            <td><?php echo $article->created_at; ?></td>
		                            <td><?php echo $article->created_user; ?></td>
		                            <td><?php echo ($article->reviewed_at != '' ? $article->reviewed_at : '-'); ?></td>
                                <td><?php echo ($article->reviewed_user != '' ? $article->reviewed_user : '-'); ?></td>
		                            <td style="color : <?=$color?>"><?php echo $status_review; ?></td>
                                <td><a onclick='View_article("<?=$article->article_id?>")' class="btn btn-info btn-anim btn-large" style="color:#fff !important;"><i class="fa fa-eye font-size-11 pt-8-im"></i></a></td>
                                <td>
                                        <?php                                            
                if($edit_access !== false && $article->created_by == $this->session->userdata('user_id')  && $article->status_review != 1){
                ?>
            <a onclick="location.href='<?=site_url('admin/article/edit/'.$article->article_id)?>'" class="btn btn-primary btn-anim btn-large" style="color:#fff !important;"><i class="fa fa-pencil font-size-11 pt-8-im"></i></a>
                <?php } ?>              
                                        </td>
                                <td>
                                        <?php    
                ...
                ?>
            <a onclick='delete_item("<?=$article->article_id?>")' class="btn btn-danger btn-anim btn-large" style="color:#fff !important;"><i class="fa fa-trash font-size-11 pt-8-im"></i></a>
                <?php } ?>                                                      
                                        </td>                               
                                <td>
                                        <?php    
                if($review_access !== false && $article->created_by != $this->session->userdata('user_id') && $article->status_review == 0){
                ?>
            <a onclick="location.href='<?=site_url('admin/article/review/'.$article->article_id)?>'" class="btn btn-success btn-anim btn-large" style="color:#fff !important;"><i class="fa fa-check font-size-11 pt-8-im"></i></a>
                <?php } ?>                                                      
                                        </td>                               
                                    </tr>
...
<script>
  ...
  function View_article(x){
        window.location.href = "<?php echo site_url('admin/article/View/')?>/"+x;
    }
</script>
</body>
</html>
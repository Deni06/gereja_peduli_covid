...
<head>
    ...
    <title>Iklan - Gereja Digital</title>
 			...
 			<div id="page-content">
                <div class="panel panel-primary"> 
                    <div class="panel-heading" style="background: #455a64">
                        <h3 class="panel-title"><b>Iklan</b></h3>
                    </div>
                    ...
                    <?php 
                            ...
                            if(($iklan == FALSE || count($iklan->result()) < 4) && $add_access !== false){ ?>
                              <button style="background: #455a64;font-family:montserrat" class="btn btn-primary btn-labeled fa fa-plus" onclick="location.href='<?=site_url('admin/iklan/add')?>'">ADD NEW</button>                         
                            <?php }
                          ?>                        
                    </div>  
                    <div>
                    ...
                    <table id="demo-dt-basic" class="table table-striped table-responsive" cellspacing="0" width="100%">
                            <thead style="font-size:1em">
                            <tr style="background: #455a64; color:white">                         
                                <th style="text-align:center">Posisi Iklan</th>                                  
                                <th width="7%"></th>     
                                <th width="7%"></th>
                                <th width="7%"></th>                            
                            </tr>
                             ...
                             <?php                                     
                             if($iklan != FALSE) {
                              foreach($iklan->result() as $i){
                                $position = "";
                                if($i->ads_position == 1){
                                  $position = "Home";
                                }else if($i->ads_position == 2){
                                  $position = "Berita";
                                }else if($i->ads_position == 3){
                                  $position = "Artikel";
                                }else{
                                  $position = "Serba - Serbi";
                                }
                                ?>
                                <tr style="font-size:1.1em; text-align:center">                                 
                                <td><?php echo $position; ?></td>		                            
                                      <td><a onclick='View_iklan("<?=$i->ads_id?>")' class="btn btn-info btn-anim btn-large" style="color:#fff !important;"><i class="fa fa-eye font-size-11 pt-8-im"></i></a></td>
                                        <td>
                                        <?php    
                if($edit_access !== false){
                ?>
                                          <a onclick="location.href='<?=site_url('admin/iklan/edit/'.$i->ads_id)?>'" class="btn btn-primary btn-anim btn-large" style="color:#fff !important;"><i class="fa fa-pencil font-size-11 pt-8-im"></i></a></td>
                <?php } ?>
                                        <td>
                                        <?php    
                if($delete_access !== false){
                ?>
                                          <a onclick='delete_item("<?=$i->ads_id?>")' class="btn btn-danger btn-anim btn-large" style="color:#fff !important;"><i class="fa fa-trash font-size-11 pt-8-im"></i></a></td>                               
                <?php } ?>
                                    </tr>
...
<script>
  ...
  function View_iklan(x){
        window.location.href = "<?php echo site_url('admin/iklan/View/')?>/"+x;
    }
</script>
</body>
</html>